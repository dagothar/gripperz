...
void GraspPlugin::updateSim() {
    ...
    if (!_simulation->isRunning()) {
        ...
        GripperQuality quality = evaluator->evaluate(_gripper, _tasks);
...
void GraspPlugin::planTasks() {
    ...
    try {
        _generator = ownedPtr(new BasicParallelGripperGraspPlanner(_nOfTargetsToGen, _td->getInitState(), _td));
...
void GraspPlugin::perturbTasks() {
    ...
    try {
        GraspFilter::Ptr robustnessFilter = new RobustnessGraspFilter(nPerturbations, 0.003, 8 * Deg2Rad);

        if (onlySuccesses) {
            _tasks = copyGrasps(_tasks, true);
        }

        _tasks = robustnessFilter->filter(_tasks);
