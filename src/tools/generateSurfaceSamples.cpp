...
int main(int argc, char* argv[]) {
	...
	string usage =
			"This is a script used to generate a database of surface samples which can be used to seed grasp target generation.\n\n"
					"Usage:\n"
					"generate-surface-samples --dwc DWC_FILE --td TD_FILE [-n N_OF_SAMPLES] [-o OUTPUT_FILE]\n";
	options_description desc("Allowed options");
	desc.add_options()("help,h", "help message")(",n",
			value<int>(&number)->default_value(1000),
			"number of samples to generate")("dwc",
			value<string>(&dwcFilename)->required(), "dynamic workcell file")(
			"td", value<string>(&tdFilename)->required(),
			"task description file")(",o",
			value<string>(&outFilename)->default_value("out.samples.xml"),
			"output file");
	...
	CollisionStrategy::Ptr cstrategy =
			ProximityStrategyFactory::makeDefaultCollisionStrategy();
	PlainTriMeshF *rayMesh = new PlainTriMeshF(1);
	(*rayMesh)[0] = Triangle<float>(Vector3D<float>(0, (float) -0.001, 0),
			Vector3D<float>(0, (float) 0.001, 0),
			Vector3D<float>(0, 0, (float) 10));
	...
	cstrategy->addGeometry(object.get(),
			td->getTargetObject()->getGeometry()[0]);
