	...
	public:
		...
		void draw(const rw::graphics::DrawableNode::RenderInfo& info, rw::graphics::DrawableNode::DrawType type, double alpha) const {
			...
			BOOST_FOREACH(Target target, _targets){
				//if(!target.enabled)
				//    continue;
				const rw::math::Vector3D<> &zoffset = _zoffset*( target.trans.R()*rw::math::Vector3D<>::z() );
				const rw::math::Vector3D<> &p = target.trans.P();
				const rw::math::Vector3D<> &pn = p+target.trans.R()*rw::math::Vector3D<>::z()*_size + zoffset;
				const rw::math::Vector3D<> &pc = p+(target.trans.R()*rw::math::Vector3D<>::x()*_size/8);
				glBegin(GL_LINES);
