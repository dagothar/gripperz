...
GripperQuality::Ptr GripperEvaluator::evaluateGripper(Gripper::Ptr gripper, rwlibs::task::GraspTask::Ptr tasks, rwlibs::task::GraspTask::Ptr samples, rwlibs::task::GraspTask::Ptr rtasks) {
    ...
    quality->certainty = calculateCertainty(tasks, samples);

    return quality;
}

double GripperEvaluator::calculateCertainty(rwlibs::task::GraspTask::Ptr tasks, rwlibs::task::GraspTask::Ptr samples) {
    INFO << "CALCULATING CERTAINTY - " << endl;
    
    int successes = TaskStatistics::countTasksWithResult(tasks, GraspResult::Success);
    int interferences = TaskStatistics::countTasksWithResult(tasks, GraspResult::Interference);
    int filteredTargets = TaskStatistics::countTasksWithResult(tasks, GraspResult::Filtered);
    int filteredSamples = TaskStatistics::countTasksWithResult(samples, GraspResult::Filtered);
    int certaintyTargets = successes + interferences;
    
    int allTargets = tasks->getAllTargets().size();
    int allSamples = samples->getAllTargets().size();
    
    INFO << "TARGET STATS -" << endl;
    INFO << " * N of TARGETS = " << allTargets << endl;
    INFO << " * N of SAMPLES = " << allSamples << endl;
    INFO << " * N of FILTERED TARGETS = " << allTargets - filteredTargets << endl;
    INFO << " * N of FILTERED SAMPLES = " << allSamples - filteredSamples << endl;
    INFO << " * N of SUCCESSES = " << successes << endl;
    INFO << " * N of COVERAGE/INTERFERENCE TARGETS = " << certaintyTargets << endl;
    
    double certainty = 1.0 * certaintyTargets / allTargets;
    return certainty;
}
