...
void readResult(PTree& tree, Gripper::Ptr gripper, const std::string& path) {
    ...
    boost::optional<PTree&> certainty = tree.get_child_optional("certainty");
    if (certainty) {
        result.certainty = XMLHelpers::readDouble(*certainty);
    }
...
void GripperXMLLoader::save(Gripper::Ptr gripper, const std::string& filename) {
    ...
    tree.put("gripper.qualities.certainty", q.certainty);
