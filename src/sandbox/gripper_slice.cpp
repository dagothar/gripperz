...
int main(int argc, char* argv[]) {
	...
	desc.add_options()
		...
		("parameters,p", value<vector<int> >(&parameters)->multitoken(), "2 parameters to scape (0-8)")
	...
	if (parameters.size() != 2) {
		RW_THROW("You must supply 2 parameters!");
	}
	...
	/* landscapes */
	ofstream dataFile(outDir + "/slice.csv");
	dataFile << paramNames[0] << ", " << paramNames[1] << ", success, robustness, alignment, coverage, wrench, stress, volume, qsum, qlog" << endl;
	...
	for (double i = bounds[params[0]].first; i < bounds[params[0]].second + 0.001; i += step1) {
		...
		for (double j = bounds[params[1]].first; j < bounds[params[1]].second + 0.001; j += step2) {
			...
			stringstream sstr;
			sstr << outDir << "/gripper_" << x << "_" << y << ".grp.xml";
